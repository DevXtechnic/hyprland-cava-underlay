...
fn main() {
    ...
    let mut custom_x: Option<i32> = None;
    let mut custom_y: Option<i32> = None;
    let mut custom_w: Option<u32> = None;
    let mut custom_h: Option<u32> = None;
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--x" => {
                if i + 1 < args.len() {
                    custom_x = Some(args[i + 1].parse().unwrap());
                    i += 2;
                }
            }
            "--y" => {
                if i + 1 < args.len() {
                    custom_y = Some(args[i + 1].parse().unwrap());
                    i += 2;
                }
            }
            "--width" => {
                if i + 1 < args.len() {
                    custom_w = Some(args[i + 1].parse().unwrap());
                    i += 2;
                }
            }
            "--height" => {
                if i + 1 < args.len() {
                    custom_h = Some(args[i + 1].parse().unwrap());
                    i += 2;
                }
            }
            _ => {
    ...
    let layer_surface = layer_shell.create_layer_surface(
        &qh,
        surface.clone(),
        Layer::Background, // Always in background for perfect underlay
        Some("wallpaper-cava"),
        None,
    );

    let final_w = custom_w.unwrap_or(256);
    let final_h = custom_h.unwrap_or(256);
    layer_surface.set_size(final_w, final_h);

    let mut anchor = Anchor::BOTTOM | Anchor::LEFT;
    if custom_x.is_none() && custom_y.is_none() && custom_w.is_none() && custom_h.is_none() {
        // Default to full background if no geometry provided
        anchor = Anchor::TOP | Anchor::LEFT | Anchor::RIGHT | Anchor::BOTTOM;
    }
    layer_surface.set_anchor(anchor);

    // Margin logic for BOTTOM | LEFT anchor:
    // x -> left margin
    // y -> bottom margin
    let mut left_m = 0;
    let mut bottom_m = 0;
    if let Some(x) = custom_x { left_m = x; }
    if let Some(y) = custom_y { bottom_m = y; }
    
    if anchor.contains(Anchor::BOTTOM) {
        layer_surface.set_margin(0, 0, bottom_m, left_m);
    } else {
        // Fallback for full-screen mode
    ...
    let mut color_keys: Vec<_> = config.colors.keys().collect();
    color_keys.sort();

    let gradient_colors_rgba: Vec<f32> = color_keys
        .iter()
        .flat_map(|key| array_from_config_color(config.colors.get(*key).unwrap().clone()))
        .collect();

    let gradient_colors_size = (gradient_colors_rgba.len() / 4) as i32;
    ...
    let gradient_colors_size_string = CString::new("gradient_colors_size").unwrap();
    let gradient_colors_string = CString::new("gradient_colors").unwrap();
    ...
    unsafe {
        ...
        let windows_size_location = gl::GetUniformLocation(shader_program, window_size_string.as_ptr());
        let gradient_colors_size_location = gl::GetUniformLocation(shader_program, gradient_colors_size_string.as_ptr());
        let gradient_colors_location = gl::GetUniformLocation(shader_program, gradient_colors_string.as_ptr());

        gl::Uniform1i(gradient_colors_size_location, gradient_colors_size);
        gl::Uniform4fv(gradient_colors_location, gradient_colors_size, gradient_colors_rgba.as_ptr());

        simple_window = AppState {
            ...
            bar_height_scale: config.bars.height_scale.unwrap_or(1.0),
...
impl AppState {
    pub fn draw(&mut self, _conn: &Connection, qh: &QueueHandle<Self>) {
        ...
        for i in 0..self.bar_count as usize {
            let bar_height: f32 = 2.0 * (unpacked_data[i] * self.bar_height_scale) - 1.0;
...
impl OutputHandler for AppState {
    ...
    fn new_output(
        ...
        if need_configuration {
            ...
            if self.custom_w.is_none() && self.custom_h.is_none() {
                ...
                self.layer_surface.set_anchor(Anchor::TOP | Anchor::LEFT | Anchor::RIGHT | Anchor::BOTTOM);
            } else {
                self.width = self.custom_w.unwrap_or(256);
                self.height = self.custom_h.unwrap_or(256);
                self.layer_surface.set_size(self.width, self.height);
                
                let anchor = Anchor::BOTTOM | Anchor::LEFT;
                self.layer_surface.set_anchor(anchor);

                let mut left_m = 0;
                let mut bottom_m = 0;
                if let Some(x) = self.custom_x { left_m = x; }
                if let Some(y) = self.custom_y { bottom_m = y; }
                self.layer_surface.set_margin(0, 0, bottom_m, left_m);
            }
            self.surface.commit();
            old_surface.destroy();
        }
